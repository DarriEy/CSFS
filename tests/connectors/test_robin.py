"""Tests for the ROBIN connector (offline EIDC/CEH global archive).

ROBIN parses a per-station local CSV (``<native_id>.csv``) with columns
``date`` (YYYY-MM-DD) and ``discharge``/``q`` (daily discharge in m3/s).
fetch_stations() returns a small seed catalogue; fetch_observations() returns
an empty chunk when no ``data_dir`` is configured.
"""
...
async def test_fetch_stations_returns_seed_list():
    """fetch_stations() returns the seed catalogue."""
    async with ROBINConnector() as conn:
        stations = await conn.fetch_stations()

    assert len(stations) > 0
    for station in stations:
        assert station.provider == "robin"
        assert station.id.startswith("robin:")

    seed = stations[0]
    assert seed.native_id == "UK_01001"
    assert seed.country_code == "GB"
    assert seed.id == "robin:UK_01001"
...
async def test_fetch_observations_no_data_dir():
    ...
    assert chunk.station_id == "robin:UK_01001"
    assert chunk.provider == "robin"
...
async def test_fetch_observations_missing_file_empty(tmp_path: Path):
    ...
    async with ROBINConnector(config=config) as conn:
        chunk = await conn.fetch_observations(
            "robin:UK_01001",
            start=datetime(2020, 1, 1, tzinfo=UTC),
            end=datetime(2020, 1, 31, tzinfo=UTC),
...
async def test_fetch_observations_from_csv(tmp_path: Path):
    """Parse a per-station daily CSV with discharge in m3/s."""
    (tmp_path / "UK_01001.csv").write_text(SAMPLE_CSV, encoding="utf-8")
...
async def test_fetch_observations_q_column_alias(tmp_path: Path):
    ...
    (tmp_path / "UK_01001.csv").write_text(
        "date,q\n2020-01-01,7.5\n", encoding="utf-8"
    )
...
def test_registration():
    """The slug resolves to the ROBINConnector class via the registry."""
    from csfs.core.registry import get_connector

    assert get_connector("robin") is ROBINConnector
