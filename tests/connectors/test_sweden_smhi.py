...
async def test_fetch_observations_filters_by_date_range():
    ...
    respx.get(
        f"{SMHI_BASE}/version/latest/parameter/1/station/1/period/latest-months/data.json"
...
async def test_fetch_observations_strips_prefix():
    ...
    respx.get(
        f"{SMHI_BASE}/version/latest/parameter/1/station/42/period/latest-months/data.json"
    ).mock(return_value=httpx.Response(200, json={"value": []}))
