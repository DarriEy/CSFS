"""LUBW Baden-Wuerttemberg connector — state hydrological service.

LUBW (Landesanstalt fuer Umwelt Baden-Wuerttemberg) operates the gauging
network for Baden-Wuerttemberg via https://udo.lubw.baden-wuerttemberg.de.
"""
...
class GermanyBWConnector(BaseConnector):
    """Connector for LUBW Baden-Wuerttemberg discharge data."""

    slug = "germany_bw"
    display_name = "LUBW Baden-Wuerttemberg"
    base_url = "https://udo.lubw.baden-wuerttemberg.de"
    country_codes = ["DE"]
    ...
    async def fetch_stations(self) -> list[Station]:
        """Return all discharge (pegel) stations from LUBW."""
        try:
    ...
    def _parse_stations(self, data: list[dict]) -> list[Station]:
        ...
        for entry in data:
            try:
                ...
                river = entry.get("river") or entry.get("gewaesser")
                area = _safe_float(
                    entry.get("catchment_area_km2")
                    or entry.get("catchment_area"),
                )
