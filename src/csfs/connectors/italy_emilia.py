"""Italy ARPAE Emilia-Romagna connector — regional hydrological data.

ARPAE (Agenzia Regionale per la Prevenzione, l'Ambiente e l'Energia)
operates the DEXT3R platform for the Emilia-Romagna region, providing
...
* Station listing:
  ...
  Returns ``[{codice, denominazione, latitudine, longitudine, bacino}]``.

* Observations:
  GET /api/data?station={codice}&variable=portata
...
class ItalyEmiliaConnector(BaseConnector):
    """Connector for ARPAE Emilia-Romagna DEXT3R discharge data."""

    slug = "italy_emilia"
    display_name = "ARPAE Emilia-Romagna (Italy)"
    base_url = "https://simc.arpae.it/dext3r"
    country_codes = ["IT"]
    ...
    async def fetch_stations(self) -> list[Station]:
        """Return all hydrometric stations from ARPAE."""
        try:
    ...
    def _parse_stations(
        ...
        for entry in items:
            ...
            lat = entry.get("latitudine") or entry.get("lat")
            lon = entry.get("longitudine") or entry.get("lon")
            ...
            try:
                stations.append(Station(
                    ...
                    provider=self.slug,
                    native_id=native_id,
                    name=entry.get("denominazione", native_id),
                    latitude=float(lat),
                    longitude=float(lon),
                    country_code="IT",
                    river=entry.get("bacino"),
                ))
    ...
    def _parse_observations(
        ...
        """Parse ARPAE observations into a ``TimeSeriesChunk``.
        ...
        May also be wrapped as ``{"data": [...]}``, ``{"values": [...]}``.
        """
        if isinstance(data, list):
            items = data
        elif isinstance(data, dict):
